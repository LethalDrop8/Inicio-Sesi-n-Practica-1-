...
// ==================== SMTP OPCIONAL ====================
// Solo se crea el transporter si hay SMTP_* en el .env
...
export const registrarUsuario = async (req, res) => {
  try {
    ...
    // Valores por defecto para columnas obligatorias
  ...
  try {
    ...
    const [rows] = await db.query(
      `SELECT id, nombre_usuario, correo
       FROM usuarios
       WHERE correo = ? AND nombre_usuario = ? AND contrasena = ?`,
...
export const forgotPassword = async (req, res) => {
  try {
    ...
    // Respondemos igual aunque no exista para no filtrar info
    if (rows.length === 0) {
      return res.json({
        ok: true,
        message:
    ...
    if (transporter) {
      // 🔵 MODO REAL: hay SMTP configurado
      try {
...
Ir a: ${appUrl}/forgot`,
      ...
      // 🧪 MODO DEMO: solo mostramos el código en consola
      console.log(
